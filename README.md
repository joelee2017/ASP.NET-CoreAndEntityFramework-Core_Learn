...
簡而言之，使用new 關鍵字創建依賴關係的實例會產生緊密耦合，因此您的應用程序將很難更改。通過依賴注入，我們不會有這種緊密耦合。

使用依賴注入，即使我們在我們的應用程序中的其他控制器中使用了相同服務，如果我們想用不同的實現交換它，我們只需要在Startup.cs文件中更改代碼。這樣帶來的效果是單元測試也變得更加容易，因為我們可以通過依賴注入輕鬆地交換依賴項。
